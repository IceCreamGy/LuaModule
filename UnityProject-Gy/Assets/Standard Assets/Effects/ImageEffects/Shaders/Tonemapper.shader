...
Shader "Hidden/Tonemapper" {
	...
	float4 fragAdaptiveAutoAvgWhite(v2f i) : SV_Target
	{
		...
		float lumScaled = cieLum * _HdrParams.z / (0.001 + avgLum.x);
		float avgWhite = avgLum.x * _AvgWhiteScale;
		lumScaled = (lumScaled * (1.0f + lumScaled / (avgWhite * avgWhite))) / (1.0f + lumScaled);
	...
	{			
		...
		float cieLum = max(0.000001, Luminance(color.rgb)); //ToCIE(color.rgb);
		//_HdrParams:
		//	z - 控制亮度倾向,来自cs->middleGrey
		//avgLum:
		//	x - 平均亮度
		//	y - 控制场景曝光度，超过y，提亮，否则变暗
		float lumScaled = cieLum * _HdrParams.z / (0.001 + avgLum.x);
		//lumScaled:ToneMapping后的亮度
		lumScaled = (lumScaled * (1.0f + lumScaled / (avgLum.y * avgLum.y)))/(1.0f + lumScaled);

		//return fixed4(cieLum, cieLum, cieLum, 1);
		//return fixed4(avgLum.x, avgLum.x, avgLum.x, 1);
		//return fixed4(avgLum.y, avgLum.y, avgLum.y, 1);
		//return fixed4(lumScaled, lumScaled, lumScaled, 1);
...
Subshader {
 ...
 Pass {
	 ...
	 // 11 adaptive reinhhard apply with auto white
 Pass{
	 ZTest Always Cull Off ZWrite Off

	 CGPROGRAM
#pragma vertex vert
#pragma fragment fragAdaptiveAutoAvgWhite
	 ENDCG
 }
}
