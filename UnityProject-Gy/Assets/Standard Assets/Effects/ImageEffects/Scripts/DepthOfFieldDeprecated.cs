...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class DepthOfFieldDeprecated : PostEffectsBase
    {
        ...
        public override bool CheckResources () {
            ...
            if(!dofBlurShader)
            {
                dofBlurShader = Shader.Find("Hidden/SeparableWeightedBlurDof34");
            }
            if(!dofShader)
            {
                dofShader = Shader.Find("Hidden/Dof/DepthOfField34");
            }
            if (!bokehShader)
            {
                bokehShader = Shader.Find("Hidden/Dof/Bokeh34");
            }
            dofBlurMaterial = CheckShaderAndCreateMaterial (dofBlurShader, dofBlurMaterial);
            dofMaterial = CheckShaderAndCreateMaterial (dofShader,dofMaterial);
            bokehSupport = bokehShader.isSupported;
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination) {
            ...
            if(foregroundTexture) foregroundTexture.DiscardContents();
            if (mediumRezWorkTexture) mediumRezWorkTexture.DiscardContents();
            if (finalDefocus) finalDefocus.DiscardContents();
            if (lowRezWorkTexture) lowRezWorkTexture.DiscardContents();
            if (bokehSource) bokehSource.DiscardContents();
            if (bokehSource2) bokehSource2.DiscardContents();
