...
Shader "QF/FX/TransparentVertexScaleDisolve_Billboard" {
	Properties{
...
#pragma fragment frag
	...
	VSOut vert(appdata v)
	{
		...
		//billboard begin
			float3 frontVector = normalize(ObjSpaceViewDir(v.vertex)).xyz;
			float3 upVector = mul(unity_WorldToObject, float4(0.0, 1.0, 0.0, 0.0)).xyz;
			float3 rightVector = cross(frontVector,upVector);
			upVector = normalize(cross(frontVector,rightVector));
			rightVector = normalize(cross(frontVector,upVector));
			float3 finalposition = v.vertex.xyz;
			finalposition += v.texcoord2.x * rightVector  * wave;
			finalposition += v.texcoord2.y * upVector  * wave;
