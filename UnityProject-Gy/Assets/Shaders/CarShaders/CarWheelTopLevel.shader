...
Shader "TopCar/Car/Car Wheel Top Level Shader" {
	Properties{
...
#pragma multi_compile USE_NORMALMAP
...
#include "UnityCG.cginc"
	...
	{
#ifdef USE_NORMALMAP
		fixed3 tangentNormal = UnpackNormal(tex2D(_NormalMap, input.tex.xy));
		float3x3 mTangentToWorld = transpose(float3x3(input.tangentWorld, input.binormalWorld, input.normalWorld));
